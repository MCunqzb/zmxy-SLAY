...
public class ColorfulThrowDaggerEffect extends AbstractGameEffect {
    private float x;
    private float y;
    private float destY;
    ...
    public ColorfulThrowDaggerEffect(float x, float y,Color c,boolean is_monster) {
    ...
    public ColorfulThrowDaggerEffect(float x, float y, float fAngle,Color c,boolean is_monster) {
        this.img = ImageMaster.DAGGER_STREAK;
        if (!is_monster){
            this.x = x - MathUtils.random(320.0F, 360.0F) - (float)this.img.packedWidth / 2.0F;
        }else {
